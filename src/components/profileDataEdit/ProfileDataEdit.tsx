...
import {ContactsType} from 'api/profile/types'

export const ProfileDataEdit: FC<ProfileDataEditPropsType> =
  ({userProfile, setEditProfile, editProfile}): ReturnComponentType => {
      ...
      <div className={style.about}>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          {contactRender.map((key) => {
            return (
              <ul key={key} className={style.contactsList}>
                <li>
                  {key}:
                  <input
                    className={errors?.contacts && style.errorInput}
                    placeholder={key}
                    {...register(`contacts.${key as keyof ContactsType}`, contactsValidate)}
                  />
                  {errors?.contacts && <p>{errors?.contacts[key as keyof ContactsType]?.message}</p>}
                </li>
              </ul>
            )
          })}
