...
export const SearchPosts: FC<SearchPostsPropsType> = ({setIsSearchPosts}): ReturnComponentType => {
  ...
  const onSearchPostsMessageChange = (event: ChangeEvent<HTMLInputElement>): void => {
  ...
  const onSearchPostsMessageKeyDown = (event: KeyboardEvent<HTMLInputElement>): void => {
  ...
  return (
    <div className={style.searchPosts}>
      <Icon20Search className={style.searchIcon} height={20} width={20}/>
      <input
      ...
      />
      <Icon12Cancel className={style.closeIcon} height={20} width={20} onClick={onResetSearchPostsMessageClick}/>
    </div>
  )
}
