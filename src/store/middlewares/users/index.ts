...
import { toggleIsLoadingAC } from "store/actions/app"
import { setCurrentPageAC, setFilterAC, setUsersAC, setTotalUsersCountAC, toggleDisabledStatusAC, followAC, unFollowAC } from "store/actions/users"
...
export const getUsersTC = (count: number, page: number, filter: any): ThunkType => async (dispatch) => {
	try {
		...
		const data = await USERS.getUsers(count, page, filter.term, filter.friend)
...
export const followTC = (userId: number): ThunkType => async (dispatch) => {
	...
	try {
		const data = await FOLLOW.follow(userId)
...
export const unFollowTC = (userId: number): ThunkType => async (dispatch) => {
	dispatch(toggleDisabledStatusAC(userId, true))
	try {
		const data = await FOLLOW.unFollow(userId)
		if (data.resultCode === 0) {
