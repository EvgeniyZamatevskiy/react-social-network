...
import { FilterType, InitialStateType } from './types'
...
const usersSlice = createSlice({
	...
	reducers: {
		setIsDisabled(state, action: PayloadAction<{ id: number, isDisabled: boolean }>) {
			...
			if (user) {
				user.disabledStatus = action.payload.isDisabled
			}
		},
	},
	extraReducers(builder) {
		builder
			.addCase(getUsers.fulfilled, (state, action: PayloadAction<{ users: UserType[], totalCount: number, page: number, filter: FilterType }>) => {
				state.users = action.payload.users.map(user => ({ ...user, disabledStatus: false }))
